...
_set_lbuffer() {
  local result selected_host connect_cmd is_skim_result
  result="$1"
  is_skim_result="$2"

  if [ "$is_skim_result" = false ] ; then
...
skim_complete_ssh() {
  ...
  if [[ "$LBUFFER" =~ "^ *ssh$" ]]; then
    zle ${skim_ssh_default_completion:-expand-or-complete}
  elif [[ "$cmd" == "ssh" ]]; then
    ...
    result=$(_skim_list_generator $result | sk \
      --height 40% \
      --ansi \
      --border \
      --cycle \
...
[ -z "$skim_ssh_default_completion" ] && {
  binding=$(bindkey '^I')
  [[ $binding =~ 'undefined-key' ]] || skim_ssh_default_completion=$binding[(s: :w)2]
  unset binding
}


zle -N skim_complete_ssh
bindkey '^I' skim_complete_ssh
