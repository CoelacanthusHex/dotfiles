...
syntax keyword birdProtocolTypes bgp device direct kernel ospf pipe rip static contained babel
...
syntax region  birdTemplate start=/^template / end=/^}/ contains=@birdCommon,@birdProtoShared,birdProtocolDirective,birdProtocolTypes,birdProtocolMisc,birdProtocolGlobals,birdImportExport,birdProtocolDisabled,birdProtocolBools,birdProtocolDate,birdFilter,birdProtocolAttributes fold
syntax keyword birdTemplateDirective template protocol area interface networks stubnet neighbors contained
syntax match   birdTemplateDirective /virtual link / contained
syntax keyword birdTemplateTypes bgp device direct kernel ospf pipe rip static contained babel
syntax keyword birdTempalteMisc as via self self; drop drop; ignore ignore; normal; large; broadcast; nonbroadcast nonbroadcast; pointopoint; none; simple; plain; md5; cryptographic; eligible; opaque; transparent; always; never; neighbor; multicast; reject; prohibit; contained
syntax keyword birdTemplateGlobals preference description id password type local neighbor multihop passive passive; persist persist; learn learn; primary rfc1583compat rfc1583compat; tick hidden hidden; summary summary; cost stub stub; hello poll retransmit priority wait authentication strict honor port infinity period mode route contained
syntax match   birdTemplateGlobals /generate from\|generate to\|accept from\|accept to\|next hop\|missing lladdr\|source address\|rr client\|rr cluster id\|rs client\|enable route refresh\|interpret communities\|enable as4\|capabilities\|advertise ipv4\|route limit\|disable after error\|\(startup \)\?hold time\|\(scan\|keepalive\|timeout\|garbage\|connect retry\|start delay\|error \(wait\|forget\)\) time\|path metric\|prefer older\|default bgp_med\|default bgp_local_pref\|device routes\|\(kernel\|peer\) table\|stub cost\|dead\( count\)\?\|rx buffer/ contained
syntax match   birdTemplateDate /\d\{2\}-\d\{2\}-\d\{4\} \d\{2\}:\d\{2\}:\d\{2\}/ contained
syntax keyword birdTemplateDisabled disabled disabled; contained
syntax keyword birdTemplateBools yes yes; no no; contained
syntax match   birdTemplateAttributes /bgp_path\|bgp_local_pref\|bgp_med\|bgp_origin\|bgp_next_hop\|bgp_atomic_aggr\|bgp_community\|bgp_originator_id\|bgp_cluster_list\|ospf_metric1\|ospf_metric2\|ospf_tag\|rip_metric\|rip_tag/ contained
highlight link birdTemplateDirective  PreProc
highlight link birdTemplateTypes      Function
highlight link birdTemplateMisc       String
highlight link birdTemplateGlobals    Constant
highlight link birdTemplateDisabled   Error
highlight link birdTemplateBools      Boolean
highlight link birdTemplateDate		  Number
highlight link birdTemplateAttributes Identifier
highlight link birdTemplate           Normal
