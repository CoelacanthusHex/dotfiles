...
update_fwdb() {
    ...
    $iucode_tool -l "$intel_tmp/Intel-Linux-Processor-Microcode-Data-Files-master/intel-ucode" | grep -wF sig | while read -r _line; do
...
write_msr() {
    ...
    if [ "$os" != Linux ]; then
        ...
        # or if we have perl, use it, any 5.x version will work
            ...
            ret=1
            perl -e "open(M,'>','/dev/cpu/$_cpu/msr') and seek(M,$_msr_dec,0) and exit(syswrite(M,pack('H16',0)))"
            [ $? -eq 8 ] && ret=0
        # fallback to dd if it supports seek_bytes
            ...
            dd if=/dev/zero of=/dev/cpu/"$_cpu"/msr bs=8 count=1 seek="$_msr_dec" oflag=seek_bytes 2>/dev/null
            ret=$?
        else
...
check_cpu() {
    ...
    if is_intel; then
        ...
        read_cpuid 0x80000008 $EBX 14 1 1
        ret=$?
        if [ $ret -eq 0 ]; then
    ...
    if is_amd || is_hygon; then
        ...
        # amd or hygon
        read_cpuid 0x80000008 $EBX 16 1 1
        ret=$?
        if [ $ret -eq 0 ]; then
            pstatus green YES
        else
        ...
        # amd or hygon
        read_cpuid 0x80000008 $EBX 18 1 1
        ret=$?
        if [ $ret -eq 0 ]; then
            pstatus green YES
        else
    ...
    if [ "$cpuid_spec_ctrl" = 1 ]; then
        ...
        read_cpuid 0x80000008 $EBX 12 1 1
        ret=$?
        if [ $ret -eq 0 ]; then
    ...
    if is_intel; then
        ...
        read_cpuid 0x80000008 $EBX 15 1 1
        ret=$?
        if [ $ret -eq 0 ]; then
    ...
    if is_amd || is_hygon; then
        ...
        read_cpuid 0x80000008 $EBX 17 1 1
        ret=$?
        if [ $ret -eq 0 ]; then
            pstatus green YES
        else
    ...
    if is_intel; then
        ...
        _info "  * Speculative Store Bypass Disable (SSBD)"
        _info_nol "    * CPU indicates SSBD capability: "
        read_cpuid 0x80000008 $EBX 24 1 1
        ret24=$?
        read_cpuid 0x80000008 $EBX 25 1 1
        ret25=$?
        if [ $ret24 -eq 0 ]; then
    ...
    if is_amd; then
        ...
        read_cpuid 0x80000008 $EBX 26 1 1
        ret=$?
        if [ $ret -eq 0 ]; then
            amd_ssb_no=1
        fi
...
check_CVE_2017_5753_linux() {
    ...
    if [ "$opt_sysfs_only" != 1 ]; then
        ...
        if [ -n "$v1_mask_nospec" ] || [ "$redhat_canonical_spectre" -gt 0 ]; then
            ...
            "${opt_arch_prefix}objdump" -d "$kernel" | perl -ne 'push @r, $_; /\s(hint|csdb)\s/ && $r[0]=~/\ssub\s+(x\d+)/ && $r[1]=~/\sbic\s+$1,\s+$1,/ && $r[2]=~/\sand\s/ && exit(9); shift @r if @r>3'
            ret=$?
            if [ "$ret" -eq 9 ]; then
